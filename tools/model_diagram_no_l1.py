"""Draw an architecture diagram of SimCLRModel (variant without L1 Norm).

Usage:
    python tools/model_diagram_no_l1.py [--proj-layers N] [--no-projector] [--out PATH]
...
def draw(proj_layers: int, no_projector: bool, out: str):
    ...
    ax.set_title("SimCLR Model Architecture (no L1 Norm)", fontsize=11, fontweight="bold", pad=8)
...
def parse_args():
    p = argparse.ArgumentParser(description="Draw SimCLR model architecture diagram (no L1 Norm)")
    ...
    p.add_argument("--out",          default="model_diagram_no_l1.png",
                   help="Output image path (default: model_diagram_no_l1.png)")
    return p.parse_args()
